...
for (panel in c('Panel1','Panel2')){
  ...
  ## Load Panel adata
  fn=paste0(proc_dir,'filtered_batch_corr_',panel,'_cells_scale_',scale_param,'_asg_conf_',avg_assignment_conf_thr,'.h5ad')
  ...
  leiden_clust_colname='scVI_leiden_subclust'#'scVI_leiden'#'leiden_merged'
  ...
  fname=paste0(panel,'_cells_scale_',scale_param,'_asg_conf_',avg_assignment_conf_thr,'_leiden_clust_marker_genes.csv')
...
# COMPARE ALL PAIRS OF CLUSTERS TO DETERMINE DE GENES BETWEEN CLUSTERS THAT ARE CLOSELY RELATED

## 1. For each Leiden-cluster-pair do:
## 2. Calculate Pseudo-bulk counts (sum counts per each patient)
## 3. Split __sce__ object into 2 datasets: cells in Leiden-clust-pair[1] vs. Leiden-clust-pair[1]
## 4. Merge the created datasets' counts and create a "coldata" dataframe for creating a design matrix later
## 5. Perform DGE with DESeq2 between Leiden-clust-pair[1] vs. Leiden-clust-pair[1] controlling for "patient" as batch effect

```{r}

## Function for searching cluster marker genes
compare_leiden_clust_pairs=function(dat,leiden_clust_pairs,coldata,col_to_sum_by,cols_to_correct_for){
  
  ## Init list for collecting results
  de_result_list=list()
  
  ## Loop over the Leiden-clusters
  for (n in 1:length(leiden_clust_pairs)){
    leiden_clust1=leiden_clust_pairs[[n]][1]
    leiden_clust2=leiden_clust_pairs[[n]][2]
    print(paste0('Leiden cluster ',leiden_clust1,' vs ',leiden_clust2))
    
    
    ### Split SCE object into cells of Leiden-cluster vs. rest
    leid_clust_dat=dat[,dat@colData[,leiden_clust_colname]==leiden_clust1]
    rest_dat=dat[,dat@colData[,leiden_clust_colname]==leiden_clust2]
    ...
    contr_data_coldata=contr_data_coldata[colnames(contr_data),]
      ...
      tryCatch({
        ...
        dds=DESeq(dge)
            ...
            message("An error occurred while processing Leiden cluster ", leiden_clust1, " vs ", leiden_clust2, ": ", e$message)
            ## Catch error and return gene counts with added +1 pesudocount
            if (grepl('every gene contains at least one zero, cannot compute log geometric means',e$message)){
              
              ## DESEq normalized by estimating the geometric mean of counts. If any gene has a 0, then geometric mean for that gene will be 0.
              #  In this error, all genes contain at least one 0 value
              # 
              #  Solution: 
              #  1 Drop genes that have only zeroes as counts 
              #  2.Add a +1 pseduocount to all counts, (log(count) will be taken anyway during normalization) => circumvent error
              
              # Drop genes with all 0 counts
              zero_rows=apply(contr_data, 1, function(row) all(row == 0))
              contr_data=contr_data[!zero_rows, ]
              
              ## Add +1 pseudocount to all values, so DESEQ can estimate a geometric mean
      ...
      df$gene=rownames(deseq_out)
      colnames(df)=c("log2FC","pvalue","padj","gene")
      df[,'comparison']=paste0(leiden_clust1,'_vs_',leiden_clust2)
      ...
      ## Add result to result list
      de_result_list[[paste0(leiden_clust1,'_vs_',leiden_clust2)]]=df
  }
...
for (panel in c('Panel1','Panel2')[1]){
  print(panel)
  ...
  ## DEFINE LEIDEN-CLUSTER PAIRS THAT WILL BE COMPARED
  leiden_clust_for_comparison=unique(adata@colData[,leiden_clust_colname])
  leiden_clust_pairs=combn(leiden_clust_for_comparison, 2, simplify = FALSE)
  
  
  marker_df_list=compare_leiden_clust_pairs(adata,leiden_clust_pairs,coldata,col_to_sum_by,cols_to_correct_for)
  ...
  fname=paste0(panel,'_cells_scale_',scale_param,'_asg_conf_',avg_assignment_conf_thr,'_all_leiden_clust_combinations.csv')
  file_path=paste0(deseq2_dirname,fname)
  write.csv(concatenated_df,file=file_path)
}
...
for (n in 1:length(leiden_clust_pairs)){
    leiden_clust1=leiden_clust_pairs[[n]][1]
    leiden_clust2=leiden_clust_pairs[[n]][2]
    print(paste0('Leiden cluster ',leiden_clust1,' vs Leiden cluster ',leiden_clust2))
}    
