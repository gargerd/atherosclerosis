...
## RUN CELLTYPE-PREDICTION WITH KNN-PREDICTOR
```{r}
proc_dir='/data/gpfs/projects/punim2121/Atherosclerosis/xenium_data/processed_data/baysor_processed_output/'
...
for (panel in c('Panel1','Panel2')){
  ...
  for (knn_neighbors in c(15,30,50)){
    
    print(paste0('Knn-neighbours ',knn_neighbors))
    ## Run KNN prediction of cell types
    query_pred=knnPredict(query,       # query object
                         reference,   # reference object
                         reference$meta_data$free_annotation, # reference cell labels for training
                         k=knn_neighbors,     # number of reference neighbors to use for prediction
                         confidence = TRUE)
...
for (panel in c('Panel1','Panel2')){
  ...
  plot_list_pred_uncert=list()
  pred_uncert_plot_n=1
  ...
  for (knn_neighbors in c(15,30,50)){
    ...
    p=ggplot(data=query_pred$meta_data,aes(x=cell_type_pred_knn_prob)) + geom_histogram(bins=20) +
          ggtitle(paste(c(panel,paste0('Cell type probabilities - ',knn_neighbors,' neighbours')),collapse=' - ')) +
          theme(plot.title = element_text(face = "bold", hjust = 0.5))  # Set font weight to bold and center the title
    
    plot_list_pred_uncert[pred_uncert_plot_n]=list(p)
    pred_uncert_plot_n=pred_uncert_plot_n+1

    unc_thr=0.8
    
    ## Init 2 new columns -> certain and uncertain
    query_pred$meta_data$certain=NA
    query_pred$meta_data$uncertain=NA
    ...
    reference$meta_data$ref_query = 'Tabula Sapiens'
    query_pred$meta_data$ref_query = 'Xenium'
    ...
    for (coln in c('donor','method')){
      ...
      plot_list_batch_corr[plot_n]=list(p1)
      plot_n=plot_n+1
      #print(p1)
    }    
    
    
    plot_list_integr=list()
    plot_n=1
    ...
    for (coln in c('ref_query','cell_type_pred_knn','cell_type_pred_knn_prob')){
      ...
      plot_list_integr[plot_n]=list(p1)
      plot_n=plot_n+1
      #print(p1)
    }
    ...
    plot_list_query_only=list()
    plot_n=1
    for (coln in c('cell_type_pred_knn','cell_type_pred_knn_prob','patient','condition')){
    
      p1=ggplot(data=query_umap,aes_string(x='UMAP1',y='UMAP2',colour=coln)) + 
             geom_point(size=0.8) + ggtitle(paste(c(panel,paste0(knn_neighbors,' neighbours')),collapse=' - '))
      p1=p1+ theme( plot.title = element_text(face = "bold", hjust = 0.5))  # Set font weight to bold and center the title
      
      plot_list_query_only[plot_n]=list(p1)
      plot_n=plot_n+1
      #print(p1)
    }
    
    print(plot_grid(plotlist=plot_list_batch_corr,ncol=1))
    print(plot_grid(plotlist=plot_list_integr,ncol=1))
    print(plot_grid(plotlist=plot_list_query_only,ncol=1))
  }
  
  print(plot_grid(plotlist=plot_list_pred_uncert,nrow=3))
}
```
...
for (panel in c('Panel1','Panel2')){
  ...
  for (leiden_merged in unique(query_pred$meta_data$leiden_merged)){
      
      df=query_pred$meta_data[query_pred$meta_data$leiden_merged==leiden_merged,]
      cell_type_freqs=(as.data.frame(table(df$cell_type_pred_knn)))
      
      p=ggplot(data=df,aes(x=cell_type_pred_knn)) + geom_bar() + ggtitle(paste(c(panel,paste0('Leiden-cluster ',leiden_merged)),collapse=' - '))+
              xlab(paste0('Predicted cell types'))
