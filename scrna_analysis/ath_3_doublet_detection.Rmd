...
```{r}
print(sce_)
a=rownames(rowData((l[[n]])))
b=(rownames(rowData((sce_))))
all(a==b)
length(colnames(sce))
length(colnames(l[[n]]))
(colnames(sce)==colnames(l[[n]]))
colnames(colData(l[[n]]))==colnames(colData(sce))
```

```{r}
c=rowData((l[[n]]))$scDblFinder.selected
d=rowData((sce_))$scDblFinder.selected
all(c==d)
```
