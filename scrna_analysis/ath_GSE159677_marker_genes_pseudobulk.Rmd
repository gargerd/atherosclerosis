...
## LOAD ANNDATA AND SUBSET IT TO HVG genes
...
## Load GSE159677 adata
fn='../../data/GSE159677_filtered_clustered.h5ad'
adata=zellkonverter::readH5AD(file=fn,reader='python')

## HVG selected by scanpy (4000 HVG) 
hvg_genes=(adata@rowRanges@elementMetadata$ENSEMBL_ID)[adata@rowRanges@elementMetadata$highly_variable]


## Extract top 8000 HVG genes with Seurat and subset SCE with it
#seur_obj=as.Seurat(adata,counts='raw_counts', data='acosh')
#seur_obj=FindVariableFeatures(object=seur_obj,nfeatures=8000)
#hvg_genes=seur_obj@assays$originalexp@var.features

```

## If marker genes are missing from the HVG genes, add them to the HVG gene list and SUBSET ADATA TO HVG + MARKER GENES
...
for (gene_dict_orig in c('high_level_ensem_gene_merged')){
...
for (gene_dict_orig in c('high_level_ensem_gene_merged')){
    ...
    hvg_plus_marker_genes=unique(c(hvg_genes,unique(unname(unlist(marker_gene_dict)))))
    print(length(hvg_plus_marker_genes))
    
}

adata_hvg=adata[hvg_plus_marker_genes,]
```
...
```{r}
## Subset metadata
meta=adata_hvg@colData

## Create a coldata dataframe containing the batch effect descriptors
coldata=aggregate(x=meta[,c('patient','condition')],
                             by=list(sample_id=meta$sample_id),FUN=unique)
rownames(coldata)=coldata$sample_id

## Define parameters of DESeq
leiden_clust_colname='scVI_leiden'#'leiden'
col_to_sum_by='sample_id'
cols_to_correct_for=c('patient','condition')

marker_df_list=get_leiden_marker_genes(adata_hvg,leiden_clust_colname,coldata,col_to_sum_by,cols_to_correct_for)
...
deseq2_dirname=paste0('../../data/','dge_deseq2/')
fname=paste0('GSE159677','_leiden_clust_marker_genes.csv.gz')
file_path=paste0(deseq2_dirname,fname)
...
for (leiden_clust in unique(dge_df$cluster)){
  ...
  p=ggplot(clust_df, aes(x = log2FC, y = -log10(padj))) +
          ...
          ggtitle(paste0('Cluster ',leiden_clust,' vs. rest')) +
...
for (gene_dict_orig in c('high_level_ensem_gene_merged')){
  ...
  for (cell_type in names(marker_gene_dict)){
    ...
    for (leiden_clust in unique(dge_df$cluster)){
      ...
      ## Extract marker genes of cluster
      clust_mark_genes=unlist(clust_df[!is.na(clust_df$DE_gene_names),'DE_gene_names'])
      
  
      clust_pos_mark_genes=intersect(clust_mark_genes,marker_gene_dict[[cell_type]])
      
      #print(paste(cell_type,leiden_clust,length(clust_pos_mark_genes)))

      d[[gene_dict_orig]][['overlapping_genes']][[cell_type]][[as.character(leiden_clust)]]=clust_pos_mark_genes
      d[[gene_dict_orig]][['num_of_overlapping_genes']][[cell_type]][[as.character(leiden_clust)]]=length(clust_pos_mark_genes)
      d[[gene_dict_orig]][['ratio_overlapping_genes']][[cell_type]][[as.character(leiden_clust)]]=length(clust_pos_mark_genes)/length(intersect(hvg_genes,unlist(marker_gene_dict[cell_type])))
      d[[gene_dict_orig]][['jaccard_index_overlapping_genes']][[cell_type]][[as.character(leiden_clust)]]=jaccard(unlist(marker_gene_dict[cell_type]),clust_pos_mark_genes)
...
```{r}
df <- as.data.frame(do.call(rbind, d[[gene_dict_orig]][['ratio_overlapping_genes']]))

numeric_dataframe <- as.data.frame(sapply(df, as.numeric))
heatmap(as.matrix(numeric_dataframe), 
        Rowv = NA,         # Don't reorder rows
        Colv = NA,         # Don't reorder columns
        col = cm.colors(256),  # Color palette (adjust as needed)
        scale = "none",  # Scale the values by column
        main = "Ratio of overlapping genes",
        xlab = "CLusters",
        ylab = "Cell types",
        labRow = row.names(df),  # Use rownames as labels
        labCol = colnames(df) ,  # Use colnames as labels
        text = TRUE,  # Show text on tiles
        cex.text = 1,  # Adjust text size as needed
        #adj = c(0.5, 0.5)  # Center the text on tiles
)
